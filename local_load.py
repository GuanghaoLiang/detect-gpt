...
##  ***** THIS IS AN VERSION OF ERIC MITCHELL'S WORK THAT HAS DISABLED DOWNLOADING MODEL AND TOKENIZER FROM INTERNET BUT
##  ***** ONLY LOAD MODELS FROM LOCAL, THE AVAILABLE MODELS ARE
##  ***** ----------GPT2
##  ***** ----------GPT2-LARGE
##  ***** ----------T5-SMALL
##  ***** ----------T5-3B
##  ***** AND ALSO DISABLED OPENAI APIS
...
def sample_from_model(texts, min_words=55, prompt_tokens=30):
    ...
    if args.dataset == 'pubmed':
        ...
        all_encoded = base_tokenizer(texts, return_tensors="pt", padding=True).to(DEVICE)
        all_encoded = {key: value[:, :prompt_tokens] for key, value in all_encoded.items()}####get the first 30 tokens for generation
...
if __name__ == '__main__':
    ...
    filepath = {'gpt2':'../autodl-tmp/gpt2', 
                'gpt2-large': '../autodl-tmp/gpt2-large',
                't5-large': '../autodl-tmp/t5-large',
                't5-small': '../autodl-tmp/models--t5-small/snapshots/3479082dc36f8a4730936ef1c9b88cd8b0835c53'}
    # API_TOKEN_COUNTER = 0
    ...
    GPT2_TOKENIZER = transformers.GPT2Tokenizer.from_pretrained(filepath['gpt2'], local_files_only = True)

    # generic generative model
    base_model, base_tokenizer = load_base_model_and_tokenizer(args.base_model_name, local_files_only = True, filepath = filepath)
    ...
    if not args.baselines_only and not args.random_fills:
        ...
        mask_model = transformers.AutoModelForSeq2SeqLM.from_pretrained(filepath[mask_filling_model_name], **int8_kwargs, **half_kwargs, local_files_only = True)
        try:
        ...
        n_positions = 512
    preproc_tokenizer = transformers.AutoTokenizer.from_pretrained(filepath['t5-small'], model_max_length=512, local_files_only = True)
    mask_tokenizer = transformers.AutoTokenizer.from_pretrained(filepath[mask_filling_model_name], model_max_length=n_positions, local_files_only = True)
